...
| -------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------ | -------- | ---------------------------------------------------------------------------------------------------------------------- |
| SEC-001  | Hardening de settings: `DEBUG=False` y `ALLOWED_HOSTS` seguros por defecto | `DEBUG` default `True` expone trazas y config en producción; `ALLOWED_HOSTS="*"` permite Host header injection                             | `config/settings.py`, `.env.example`                   | S        | `DEBUG` default `False`; `ALLOWED_HOSTS` solo `localhost,127.0.0.1`; activables explícitamente por `.env`              |
...
| ID       | Título                                                        | Hallazgo                                                                                                             | Archivos afectados                                                           | Esfuerzo | Criterios de aceptación esperados                                                                              |
| -------- | ------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- | -------- | -------------------------------------------------------------------------------------------------------------- |
| CONF-001 | Configurar `devtools.yaml` con comandos reales                | `testing.command`, `testing.path`, `quality.lint_command` vacíos → `tdd.py` y `preflight.py` no funcionan            | `devtools.yaml`                                                              | S        | `python "$DEVTOOLS_HOME/bin/preflight.py" -d .` sale con código 0                                              |
